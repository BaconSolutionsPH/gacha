...
export const NonceResponseSchema = t.Object({
    nonce: t.String(),
});
export const GetNonceRequestSchema = t.Object({
    address: t.String(),
});
...
export namespace AuthController {
    export const login = async (
        params: (typeof LoginRequestSchema)["static"],
    ) => {
    ...
    export const getUser = async (authToken?: string) => {
        ...
        const jwtToken = jwt.verify(authToken, envConfig.JWT_SECRET) as
            | (typeof LoginResponseSchema)["static"]["user"]
            | null;
        if (!jwtToken) {
            return status(400, { message: "Invalid token." });
        }
        const userData = await UserModel.getUserOrCreateUser(
            jwtToken.walletAddress,
        );
        if (!userData) {
